...
export function CatalogManager() {
  const { catalogs, loading, create, update, remove } = useCatalogs();
  ...
  return (
    <div className="p-8">
          ...
          {catalogs.map((c) => (
            <div key={c.id} className="flex items-center gap-4 rounded-xl border bg-white p-4 shadow-sm">
              ...
              <div className="flex gap-2">
                <button onClick={() => openEdit(c)} className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-vega-blue">
      ...
      <FormDialog open={formOpen} onClose={() => { setFormOpen(false); setEditCatalog(null); setForm({}); }} title={editCatalog ? "Edit Catalog" : "Add Catalog"} onSubmit={handleSubmit}>
