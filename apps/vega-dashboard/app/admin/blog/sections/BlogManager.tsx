...
export function BlogManager() {
  const { blogs, loading, create, remove } = useBlog();
  ...
  return (
    <div className="p-8">
      <PageHeader title="Blog Manager" subtitle="Create and publish blog posts." actionLabel="Add Post" onAction={() => setFormOpen(true)} />
          ...
          {filtered.map((b) => (
            <div key={b.id} className="flex items-center gap-4 rounded-xl border bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
              ...
              <div className="flex gap-2">
                <button className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 hover:text-vega-blue"><Edit2 className="h-4 w-4" /></button>
      ...
      <FormDialog open={formOpen} onClose={() => setFormOpen(false)} title="Add Blog Post" onSubmit={handleCreate}>
        <div className="space-y-4">
          ...
          <ImageUpload folder="blog" onChange={(url) => updateForm("featuredImage", url)} label="Featured Image" />
          <div>
