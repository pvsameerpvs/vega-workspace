...
export function GalleryManager() {
  ...
  return (
    <div className="p-8">
            ...
            <div key={item.id} className="group relative overflow-hidden rounded-xl border bg-white shadow-sm">
              ...
              <div className="absolute inset-0 flex items-end justify-between bg-gradient-to-t from-black/60 to-transparent p-4 opacity-0 transition-opacity group-hover:opacity-100">
                ...
                <div className="flex gap-2">
                  <button onClick={() => openEdit(item)} className="rounded-md bg-white p-1.5 text-slate-700 hover:bg-slate-100">
      ...
      <FormDialog open={formOpen} onClose={() => { setFormOpen(false); setEditItem(null); setForm({}); }} title={editItem ? "Edit Gallery Image" : "Add Gallery Image"} onSubmit={handleSubmit}>
