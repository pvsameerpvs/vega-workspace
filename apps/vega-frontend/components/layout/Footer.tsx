    ...
    <footer className="bg-[#0f172a] text-white/50" dir={isAR ? "rtl" : "ltr"}>
      <div className="mx-auto max-w-7xl px-4 py-12">
        <div className={`grid gap-10 md:grid-cols-2 lg:grid-cols-4 ${isAR ? "text-right" : ""}`}>
          ...
          {/* Products */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">{isAR ? "المنتجات" : "Products"}</h4>
          ...
          {/* Company */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">{isAR ? "الشركة" : "Company"}</h4>
            <ul className="space-y-2">
              {companyLinks.map((item) => (
                <li key={item.path}>
                  <Link href={l(item.path)} className="text-sm hover:text-[#FFD400] transition-colors">
                    {item.label}
                  </Link>
                </li>
              ))}
          ...
          {/* Contact */}
          <div>
            <h4 className="text-sm font-semibold text-white mb-4">{isAR ? "تواصل معنا" : "Contact"}</h4>
        ...
        <div className={`mt-10 flex flex-col md:flex-row items-center gap-4 border-t border-white/10 pt-6 ${isAR ? "flex-row-reverse justify-between" : "justify-between"}`}>
          ...
          <div className="flex gap-4">
            <Link href={l("/contact-us")} className="rounded-full bg-[#FFD400] px-5 py-2 text-xs font-bold text-[#1F3A93] hover:bg-white transition-all whitespace-nowrap">
              {isAR ? "اطلب عرض سعر" : "Request a Quote"}
            </Link>
            <Link href={l("/catalog")} className="rounded-full border border-white/20 px-5 py-2 text-xs text-white/70 hover:bg-white/10 hover:text-white transition-all whitespace-nowrap">
