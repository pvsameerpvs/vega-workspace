...
export function ShareSection({ locale }: ShareSectionProps) {
  const isAR = locale === "ar";
  const pageUrl = `https://www.thevegauae.com/${locale}/contact-us`;

  const shareLinks = [
    {
      name: "WhatsApp",
      href: `https://wa.me/971567351095?text=${encodeURIComponent(pageUrl)}`,
    ...
    {
      name: "X",
      href: `https://twitter.com/intent/tweet?text=${encodeURIComponent("Contact Vega UAE for furniture, barriers & camp supplies")}&url=${encodeURIComponent(pageUrl)}`,
    ...
    <div className="flex items-center gap-2">
      {shareLinks.map((s) => (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 rounded-full bg-slate-100 px-3 py-1.5 text-xs font-semibold text-slate-600 hover:bg-[#1F3A93] hover:text-white transition-all"
        >
