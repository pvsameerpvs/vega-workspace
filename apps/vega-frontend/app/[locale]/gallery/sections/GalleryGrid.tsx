...
export function GalleryGrid({ items, locale = "en" }: GalleryGridProps) {
  ...
  const categories = useMemo(() => {
    ...
    return [isAR ? "الكل" : "All", ...Array.from(set)];
  }, [items, isAR]);

  const [activeFilter, setActiveFilter] = useState(isAR ? "الكل" : "All");
    ...
    <>
      ...
      <div className="mb-12 flex flex-wrap gap-3">
        {categories.map((cat) => (
          ...
          >
            {cat === allLabel && <ImageIcon className="h-4 w-4" />}
            {cat}
          </button>
        ))}
      </div>
      ...
      {filtered.length === 0 && (
        <div className="py-24 text-center">
          ...
          <p className="text-base text-slate-400">{isAR ? "لا توجد صور في هذه الفئة." : "No images in this category."}</p>
        </div>
      )}
        ...
        >
          ...
          >
            <img
              ...
              alt={isAR && selectedImage.nameAr ? selectedImage.nameAr : selectedImage.name}
            ...
            <div className="absolute bottom-4 left-4 rounded-xl bg-black/60 px-4 py-2 backdrop-blur-md">
              ...
              <p className="text-sm font-bold text-white">{isAR && selectedImage.nameAr ? selectedImage.nameAr : selectedImage.name}</p>
            </div>
          </div>
        </div>
      )}
