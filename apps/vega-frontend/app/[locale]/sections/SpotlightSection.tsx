...
export async function SpotlightSection({ locale = "en" }: SpotlightSectionProps) {
    ...
    <section className="py-12 bg-white">
      <div className="mx-auto max-w-7xl px-4">
        <div className="mb-8 flex items-center gap-3">
          ...
          <span className="text-xs font-bold uppercase tracking-[0.2em] text-[#FFD400]">{isAR ? "مميز" : "Featured"}</span>
        </div>
        <h2 className="section-heading text-3xl md:text-4xl mb-10">{isAR ? "في دائرة الضوء" : "In the Spotlight"}</h2>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {/* First item — large (2x2 on desktop) */}
          <Link
            href={l(`/products/${first.link}`)}
